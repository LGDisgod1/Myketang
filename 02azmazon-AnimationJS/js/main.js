...
for (var i = 0; i < galleryInfo.imageList.length; i++) {//遍历imgagelist
	var nodeImg = document.createElement("img"); //创建一个img的节点
	nodeImg.className = "pic opacity0";//将图像节点附加到i里
	nodeImg.src = galleryInfo.imgPath + galleryInfo.imageList[i].src;
	var nodeA = document.createElement("a");//创建一个A节点
	nodeA.href = galleryInfo.imageList[i].href;
	nodeA.appendChild(nodeImg);
	var nodeLiImg = document.createElement("li");//创建一个Li节点
